...
export default function BlogListing({ initialPosts }: BlogListingProps) {
  ...
  const filteredPosts =
    selectedTag === "All"
      ? initialPosts.filter((post) => !post.isTopPick)
      : initialPosts.filter((post) => !post.isTopPick && post.tag === selectedTag);

  const topPicks = initialPosts.filter((post) => post.isTopPick);
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3 text-sm text-label">
                      <time dateTime={new Date(post.date).toISOString()} className="font-medium">
                        {post.date}
                      </time>
